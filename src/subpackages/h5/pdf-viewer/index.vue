<template>
  <view class="pdf-page">
    ...
    <!-- 无内容 -->
    <view v-else-if="pages.length === 0" class="pdf-status">
      <text class="pdf-status-text">暂无内容</text>
    </view>

    <!-- HTML 渲染（合同预览） -->
    <view v-else-if="htmlContent" class="pdf-html">
    ...
    <!-- PDF 图片展示 -->
    <view v-else class="pdf-content">
      <view class="pdf-header">
        <text class="pdf-title">{{ title }}</text>
        <text class="pdf-page-info">{{ pages.length }} 页</text>
      </view>
      <scroll-view class="pdf-scroll" scroll-y="true">
        <view v-for="page in pages" :key="page.pageNum" class="pdf-page-item">
          <image
            class="pdf-page-img"
            :src="page.imageUrl"
            mode="widthFix"
            lazy-load
            @error="onImgError(page.pageNum)"
          />
          <text class="pdf-page-num">{{ page.pageNum }}</text>
        </view>
      </scroll-view>
    </view>
  </view>
...
import { getPdfByToken, type PdfPage } from '@/api/pdf'
...
onMounted(async () => {
  ...
  const viewerType = instance.router?.params?.type || ''
  ...
  try {
    if (viewerType === 'contract') {
      ...
      // 普通 PDF：调用原 PDF 接口
      const doc = await getPdfByToken(token)
      title.value = doc.title || ''
      pages.value = doc.pages || []
    }
  } catch {
...
const onImgError = (pageNum: number) => {
  Taro.showToast({ title: `第${pageNum}页加载失败`, icon: 'none' })
}
